...
def wikihop_vocab_collection(args):
    ...
    processed_train_data_name = join(PREPROCESS_FOLDER, args.process_train_name)
    processed_dev_data_name = join(PREPROCESS_FOLDER, args.process_dev_name)
    ...
    _, fasttext_word2idx = load_pretrained_embedding_ndarray(embeding_file_name=fasttext_model_name)
    # glove_model_name = join(PRETRAINED_MODEL_FOLDER, args.glove_model)
    # _, glove_word2idx = load_pretrained_embedding_ndarray(embeding_file_name=glove_model_name)
    oov_count = 0
    for key, value in vocab_dict.items():
        if (key not in fasttext_word2idx):
            print(key, value)
            oov_count = oov_count + 1
    print(len(vocab_dict), oov_count)

def restore_fast_text_word_embeddings(args):
    fasttext_model_name = join(PRETRAINED_MODEL_FOLDER, args.fasttext_model)
...
def restore_glove_word_embeddings(args):
