...
def sliding_window_fast(seq_len: int, start_offset=0, window_size=24):
    ...
    assert len(sliding_src_array) == len(sliding_dst_array)
...
def graph_triple_construction(seq_len: int, start_offset: int, window_size, global_idx: list):
    ...
    rel_list = [np.full(sliding_len, 1, dtype=int), np.full(global_len, 2, dtype=int),
                np.full(global_len, 2, dtype=int)]
...
def seq2graph(sequence: list, global_idx: list, start_offset: int = 0, window_size=2) -> DGLHeteroGraph:
    """
    :param sequence:
    :param global_idx:
    :param window_size:
    ...
    src, dst, rel = graph_triple_construction(seq_len=number_of_nodes, global_idx=global_idx, window_size=window_size,
